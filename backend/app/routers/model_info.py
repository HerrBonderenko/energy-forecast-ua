...
# ─── Реальне перенавчання моделі ─────────────────────────────────────────────

@router.post("/retrain")
def retrain_model():
    """Запускає реальне перенавчання моделі у фоновому потоці."""
    started = start_retrain_background()
    if not started:
        return {
            "started": False,
            "message": "Навчання вже триває",
            "state":   get_training_state(),
        }
    return {
        "started": True,
        "message": "Навчання розпочато. Перевіряйте статус через /api/model/retrain/status",
        "state":   get_training_state(),
    }
